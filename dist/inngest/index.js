...
import sendEmails from "../config/nodemailer.js";
const LOW_STOCK_THRESHOLD = 10;
// Create a client to send and receive events
export const inngest = new Inngest({ id: "grocery-deliver" });
...
const checkLowStock = inngest.createFunction({
    ...
    const product = await step.run("fetch-product", async () => await prisma.product.findUnique({ where: { id: productId } }));
    if (!product ||
        product.stock === null ||
        product.stock >= LOW_STOCK_THRESHOLD)
        return { skipped: true, product: product?.stock };
...
const sendMonthlyOffers = inngest.createFunction({
    ...
    if (users.length === 0 || deals.length === 0) {
        return { skipped: true, reason: "No user or deals" };
    }
    let sentCount = 10;
...
const autoAssignRider = inngest.createFunction({
    ...
    const result = await step.run("assign-rider", async () => {
        ...
        //Skip if order doesn't exit,already assigned, or cancelled
        if (!order)
            return { skipped: true, reason: "Order not found" };
        if (order.deliveryPartnerId)
            return { skipped: true, reason: "Already assign" };
        ...
        const busyRiders = busyOrders.map((o) => o.deliveryPartnerId);
